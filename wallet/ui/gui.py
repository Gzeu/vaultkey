"""
gui.py — CustomTkinter GUI for VaultKey (Wave 8 — synced with Wave 7 features).

Tabs:
  Keys       — Scrollable card list, live search, Copy/Info/Delete/Rename per card.
  Expiry     — Color-coded expiry warnings with urgency levels (Wave 7).
  Health     — Per-entry health scores, overall grade, recommendations.
  Settings   — Change password, export/import, bulk import, audit viewer.
...
- Wave 7 additions: Expiry tab, Bulk Import panel, Rename button on key cards.
"""
...
class RenameDialog(ctk.CTkToplevel):
    """Modal dialog to rename an existing key entry (Wave 7)."""

    def __init__(self, parent, entry: APIKeyEntry, on_rename) -> None:
        super().__init__(parent)
        self.title(f"Rename — {entry.name}")
    ...
    def _build_ui(self) -> None:
        ctk.CTkLabel(self, text="New name:", anchor="w").pack(fill="x", padx=20, pady=(20, 4))
        self._var = ctk.StringVar(value=self._entry.name)
        self._input = ctk.CTkEntry(self, textvariable=self._var, width=340)
        self._input.pack(padx=20)
        self._input.select_range(0, "end")
        self._input.focus_set()
        self._input.bind("<Return>", lambda _: self._save())

        self._error = ctk.CTkLabel(self, text="", text_color="#F44336")
        self._error.pack(pady=4)

        ctk.CTkButton(self, text="✓ Rename", height=36, width=140,
...
class KeyCard(ctk.CTkFrame):
    ...
    def __init__(self, parent, entry: APIKeyEntry, on_copy, on_info, on_rename, on_delete) -> None:
        ...
        if entry.expires_at:
            exp_str = entry.expires_at.strftime("%Y-%m-%d")
            ctk.CTkLabel(left, text=f"Expires: {exp_str}",
                          text_color="#9E9E9E", anchor="w").pack(fill="x")
        ...
        ctk.CTkButton(right, text="Rename", width=64, height=28,
                       fg_color="#2a4a6a", hover_color="#1a3a5a",
                       command=lambda: on_rename(entry)).pack(pady=2)
...
class KeysTab(ctk.CTkFrame):
    ...
    def _rename_key(self, entry: APIKeyEntry) -> None:
        """Open rename dialog and persist the new name (Wave 7)."""
        RenameDialog(self, entry, self._do_rename)

    def _do_rename(self, entry: APIKeyEntry, new_name: str) -> None:
        try:
            self._payload.rename_entry(entry.name, new_name)
        except (KeyError, ValueError) as e:
            # Show error in a simple dialog
            err = ctk.CTkToplevel(self)
            err.title("Rename Error")
            err.geometry("340x120")
            ctk.CTkLabel(err, text=str(e), wraplength=300).pack(pady=20)
            ctk.CTkButton(err, text="OK", command=err.destroy).pack()
            return
        storage.save(self._master_key, self._params, self._payload.to_dict())
...
class ExpiryTab(ctk.CTkFrame):
    """Color-coded expiry warnings for all keys with set expiry dates."""
    ...
    def _build_ui(self) -> None:
        ...
        top = ctk.CTkFrame(self, fg_color="transparent")
        top.pack(fill="x", padx=8, pady=(8, 4))
        ctk.CTkLabel(top, text="Look-ahead (days):", width=140, anchor="w").pack(side="left")
        self._days_var = ctk.StringVar(value="30")
        ctk.CTkEntry(top, textvariable=self._days_var, width=60).pack(side="left")
        ctk.CTkButton(top, text="Refresh", width=80,
                       command=self._refresh).pack(side="left", padx=8)
        self._show_all_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(top, text="Show all",
    ...
    def _refresh(self) -> None:
        ...
        if not self._show_all_var.get():
            warnings = [w for w in warnings if w.urgency in ("expired", "critical", "warning")]

        if not warnings:
            ctk.CTkLabel(self._scroll,
                          text="✅ No expiry warnings in this window.",
                          text_color="#4CAF50").pack(pady=20)
            self._summary.configure(text="All clear")
            return

        expired = sum(1 for w in warnings if w.urgency == "expired")
        critical = sum(1 for w in warnings if w.urgency == "critical")
        warning = sum(1 for w in warnings if w.urgency == "warning")
        self._summary.configure(
            text=f"🔴 {expired} expired  🟠 {critical} critical  🟡 {warning} warning"
        )

        for ew in warnings:
            color = URGENCY_COLORS.get(ew.urgency, "white")
            card = ctk.CTkFrame(self._scroll, corner_radius=6, border_width=1,
                                 border_color="#333")
            ...
            urgency_icon = {"expired": "🔴", "critical": "🟠", "warning": "🟡", "info": "⚪"}
            icon = urgency_icon.get(ew.urgency, "")

            ctk.CTkLabel(row, text=f"{icon} {ew.name}",
                          font=("Helvetica", 12, "bold"),
                          text_color=color, width=220, anchor="w").pack(side="left")

            if ew.days_left < 0:
                days_text = f"Expired {abs(ew.days_left)} day(s) ago"
            elif ew.days_left == 0:
                days_text = "Expires TODAY"
            else:
                days_text = f"{ew.days_left} day(s) remaining"

            ctk.CTkLabel(row, text=days_text,
                          text_color=color, anchor="w").pack(side="left", padx=12)
            ctk.CTkLabel(row,
                          text=ew.expires_at.strftime("%Y-%m-%d") if ew.expires_at else "—",
...
# ------------------------------------------------------------------ #
# Settings Tab (with Bulk Import panel)
# ------------------------------------------------------------------ #

class SettingsTab(ctk.CTkFrame):
    """Change password, export backup, bulk import, view audit log."""
    ...
    def _build_ui(self) -> None:
        ...
        sec1.pack(fill="x", padx=4, pady=(8, 4))
        ...
        sec2.pack(fill="x", padx=4, pady=4)
        ...
        sec3.pack(fill="x", padx=4, pady=4)
        ctk.CTkLabel(sec3, text="Bulk Import (.env / .json / .csv)",
                      font=("Helvetica", 13, "bold")).pack(anchor="w", padx=10, pady=6)

        ctrl = ctk.CTkFrame(sec3, fg_color="transparent")
        ctrl.pack(fill="x", padx=10, pady=2)
        ctk.CTkLabel(ctrl, text="On conflict:", width=90, anchor="w").pack(side="left")
        self._conflict_var = ctk.StringVar(value="skip")
        for strategy in ("skip", "overwrite", "rename"):
            ctk.CTkRadioButton(ctrl, text=strategy, variable=self._conflict_var,
                                value=strategy).pack(side="left", padx=6)

        self._dry_run_var = ctk.BooleanVar(value=True)
        ctk.CTkCheckBox(sec3, text="Dry run (preview only)",
                         variable=self._dry_run_var).pack(anchor="w", padx=10)

        self._bulk_result = ctk.CTkLabel(sec3, text="", wraplength=480, justify="left")
        self._bulk_result.pack(anchor="w", padx=10, pady=4)
        ctk.CTkButton(sec3, text="Choose file…", width=140,
                       command=self._bulk_import).pack(padx=10, pady=(0, 8))

        # — Audit Log —
        sec4 = ctk.CTkFrame(scroll)
        sec4.pack(fill="x", padx=4, pady=4)
        ctk.CTkLabel(sec4, text="Recent Audit Events (last 20)",
                      font=("Helvetica", 13, "bold")).pack(anchor="w", padx=10, pady=6)

        audit_scroll = ctk.CTkScrollableFrame(sec4, height=160)
    ...
    def _bulk_import(self) -> None:
        """Open a file picker and run bulk import (Wave 7)."""
        from tkinter import filedialog
        path = filedialog.askopenfilename(
            title="Select secrets file",
            filetypes=[
                ("All supported", "*.env *.json *.csv"),
                (".env files", "*.env"),
                ("JSON files", "*.json"),
                ("CSV files", "*.csv"),
            ],
        ...
        try:
            ...
            self._bulk_result.configure(
                text=f"❌ Parse error: {e}", text_color="#F44336"
            )
            return
        ...
        if not dry_run:
            ...
            audit_log("BULK_IMPORT", status="OK",
                      extra=f"added={result.added} skipped={result.skipped}")

        prefix = "[DRY RUN] " if dry_run else ""
        self._bulk_result.configure(
            text=(
                f"{prefix}✅ Added: {result.added}  "
                f"Skipped: {result.skipped}  "
                f"Overwritten: {result.overwritten}  "
                f"Renamed: {result.renamed}  "
                f"Errors: {result.errors}"
            ),
            text_color="#4CAF50" if not result.errors else "#FF9800",
        )
...
class MainWindow(ctk.CTk):
    """Tabbed main window: Keys | Expiry | Health | Settings."""
    ...
    def _build_ui(self) -> None:
        ...
        self._expiry_tab = ExpiryTab(
            self._tabs.tab("⏰  Expiry"),
            self._payload,
        )
        self._expiry_tab.pack(fill="both", expand=True)
    ...
    def _on_data_changed(self) -> None:
        """Rebuild Expiry and Health tabs after any data mutation."""
        for tab_name, cls in [
            ("⏰  Expiry", ExpiryTab),
            ("📊  Health", HealthTab),
        ]:
            for w in self._tabs.tab(tab_name).winfo_children():
                w.destroy()
        self._expiry_tab = ExpiryTab(self._tabs.tab("⏰  Expiry"), self._payload)
        self._expiry_tab.pack(fill="both", expand=True)
